  ...
  public:
...
#if defined (CCP)
      CCP = 0xD8;							// Разблокировка доступа к watchdog, ATtiny10
#else
	  WDT_REG = ((1 << WDCE) | (1 << WDE)); // Разблокировка доступа к watchdog, см. документацию на МК
#endif
      WDT_REG = 0x00;						// Очищаем все настройки watchdog
    ...
    */
    ...
    void enable(uint8_t mode, uint8_t prescaler) {  // Настроить и запустить watchdog таймер
      uint8_t _WDT_CFG = mode | ((prescaler > 7) ? ((1 << WDP3) | (prescaler - 8)) : prescaler); // Cоставляем содержимое регистра WDT
      uint8_t _SREG_COPY = SREG;				 	// Сохраняем SREG (и настройки глобальных прерываний)
      noInterrupts();				 				// Запрещаем глобальные прерывания
#if defined (CCP)
      CCP = 0xD8;									// Разблокировка доступа к watchdog, ATtiny10
	  ...
	  WDT_REG = ((1 << WDCE) | (1 << WDE));  		// Разблокировка доступа к watchdog, см. документацию на МК
#endif
      WDT_REG = _WDT_CFG;				 			// Сразу же загружаем готовые настройки watchdog
      SREG = _SREG_COPY;							// Возвращаем SREG и прерывания, если они были включены до этого	
    }
    /*
